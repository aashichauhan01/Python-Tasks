...
class ContactBook:
    def __init__(self, window):
        ...
        # UI Elements
        self.label = tk.Label(window, text="Contact Book", font=("Helvetica", 16))
        self.label.grid(row=0, column=1, pady=10)

        self.add_button = tk.Button(window, text="Add Contact", command=self.add_contact)
        self.add_button.grid(row=1, column=0, padx=10, pady=10)

        self.view_button = tk.Button(window, text="View Contacts", command=self.view_contacts)
        self.view_button.grid(row=1, column=1, padx=10, pady=10)

        self.search_button = tk.Button(window, text="Search Contact", command=self.search_contact)
        self.search_button.grid(row=1, column=2, padx=10, pady=10)

        self.update_button = tk.Button(window, text="Update Contact", command=self.update_contact)
        self.update_button.grid(row=2, column=0, padx=10, pady=10)

        self.delete_button = tk.Button(window, text="Delete Contact", command=self.delete_contact)
        self.delete_button.grid(row=2, column=1, padx=10, pady=10)

        self.exit_button = tk.Button(window, text="Exit", command=window.destroy)
    ...
    def view_contacts(self):
        ...
        if not self.contacts:
            ...
            for i, contact in enumerate(self.contacts, start=1):
                contact_info = f"{i}. {contact['Store Name']} - {contact['Phone Number']} - {contact['Email']} - {contact['Address']}"
